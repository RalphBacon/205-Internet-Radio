/*
	...
	2 -	Do the Next and Prev buttons properly, possibly via interrupts but we must
		ensure not to impact the data stream buffering (too much) FIXME DONE:
	...
	7 - Ensure critical errors are displayed on screen (eg WiFi problem, station not
		connected to after X attempts) as user not aware of problems FIXME:
	...
	10 - Mute sound from any station until we have played at least XXX mS to avoid the
		...
		whether buffering clears this anomaly FIXME:
	...
	20 - If corrupt METADATA is discovered do NOT clear the ring buffer, that data is good FIXME:

	21 - On changing station clear the ring buffer otherwise it continues to play that until new data is found
