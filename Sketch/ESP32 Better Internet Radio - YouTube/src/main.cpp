...
void loop() {
    ...
    if (currDisplayScreen == HOME) {
        ...
        // Settings Button (bass, treble etc) (work in progress)
        getSettingsBtn();
...
void readHttpStream() {
    ...
    if (bufferPerCent < 5) {
        ...
        if (millis() - circBufferLow > LOWBUFFER_SECONDS) {
            ...
            log_d("Low buffer: %d%%, occurrence %d", bufferPerCent, ++lowCircBuffCnt);
            ...
            if (lowCircBuffCnt >= LOWBUFFER_RECOVERYCOUNT_MAX) {
                // RSB Dec 2021
                log_v("Low buffer: flushing buffer, resetting flags");
                lowCircBuffCnt = 0;
                circBuffer.flush();
                ...
                log_w("Low buffer: attempting to reconnect to current station");
                changeStation(currStnNo, true);
            }
        }
    } else {
        ...
        if (lowCircBuffCnt > 0) {
            log_d("Low buffer: recovered, now %d%%", bufferPerCent);
            lowCircBuffCnt = 0;
        }
    }
}
...
bool stationConnect(int stationNo, bool clearBuffer) {
    ...
    // RSB Dec 2020 Set the flag that we are connected - now we will be able to read the buffer
    connectedToStation = true;
    displayTrackArtist("Connected to station. Reading stream...");
